...
-> ADDED: Scrollbar and Mathematical Draw Clipping
]]
    ...
    function UILib:Step()
        ...
        if self._menu_open then
            ...
            for i = 1, tabCount do
                ...
                if isOpen then
                    -- SCROLLBAR LOGIC
                    local maxContentHeight = tabContent._max_height or 0
                    local maxScroll = math.max(0, maxContentHeight - (bodyContentSize.y - self._tab_h - self._padding))
                    tabContent._scroll = clamp(tabContent._scroll or 0, 0, maxScroll)
                    
                    local scrollbarWidth = 0
                    if maxScroll > 0 then
                        scrollbarWidth = 6
                        local sbX = bodyContentPos.x + bodyContentSize.x - scrollbarWidth - 2
                        local sbY = bodyContentPos.y + self._tab_h + 2
                        local sbH = bodyContentSize.y - self._tab_h - 4
                        local thumbH = math.max(20, ((bodyContentSize.y - self._tab_h) / maxContentHeight) * sbH)
                        local thumbY = sbY + (tabContent._scroll / maxScroll) * (sbH - thumbH)

                        local isHoverSB = self:_IsMouseWithinBounds(Vector2.new(sbX - 2, sbY), Vector2.new(scrollbarWidth + 4, sbH))
                        if mouseHeld then
                            if clickFrame and isHoverSB then
                                self._scrollbar_drag = tabName
                                clickFrame = false
                            end
                            if self._scrollbar_drag == tabName then
                                local my = self:_GetMousePos().y
                                local pct = clamp((my - sbY - thumbH/2) / (sbH - thumbH), 0, 1)
                                tabContent._scroll = pct * maxScroll
                                
                                -- Auto close dropdowns & colorpickers while scrolling to prevent position drift
                                self:_RemoveDropdown()
                                self:_RemoveColorpicker()
                            end
                        else
                            if self._scrollbar_drag == tabName then self._scrollbar_drag = nil end
                        end

                        self._clip_box = nil -- don't clip scrollbar
                        self:_Draw('menu_scrollbar_bg', 'rect', self._theming.surface1, 25, Vector2.new(sbX, sbY), Vector2.new(scrollbarWidth, sbH), true)
                        self:_Draw('menu_scrollbar_thumb', 'rect', self._theming.border1, 26, Vector2.new(sbX, thumbY), Vector2.new(scrollbarWidth, thumbH), true)
                    else
                        self:_Undraw('menu_scrollbar_bg')
                        self:_Undraw('menu_scrollbar_thumb')
                        tabContent._scroll = 0
                    end

                    -- Activate mathematical clipping for content area
                    ...
                    local sectionIter = 0
                    local sectionWidth = (bodyContentSize.x - (maxScroll > 0 and (scrollbarWidth + 4) or 0)) / 2 - self._padding * 1.5
                    local totalSectionHeightR = self._padding * 1.5
                    local totalSectionHeightL = self._padding * 1.5
                    local scrollOffset = tabContent._scroll
                    ...
                    for sIdx = 1, #tabContent._section_order do
                        ...
                        if isSectionMirror then
                            sectionPos = sectionPos + Vector2.new(sectionWidth + self._padding, totalSectionHeightR + sectionTitleSize.y/2 - scrollOffset)
                        else
                            sectionPos = sectionPos + Vector2.new(0, totalSectionHeightL + sectionTitleSize.y/2 - scrollOffset)
                        end
                        ...
                        for sectionItemIter, sectionItem in ipairs(sectionContent._items) do
                            ...
                            if itemType == 'toggle' then
                                ...
                                if sectionItem.tooltip then
                                    ...
                                    if isHoveringHint then
                                        ...
                                        -- disable clip box temporarily so tooltip renders fully over menu
                            ...
                            elseif itemType == 'slider' then
                                ...
                                self:_Draw(sectionItemId .. '_slider', 'gradient', nil, 20, 'vertical', sliderOrigin + Vector2.new(1, 1), Vector2.new(math.max(1, sliderSize.x * fillPercent - 2), sliderSize.y - 2), tickColor)
                        ...
                        -- Dynamic stretching ONLY if space remains on the screen
                        if isLastSection then
                            if isSectionMirror then
                                local remaining = (bodyContentSize.y - self._tab_h) - totalSectionHeightR
                                if remaining > 0 then sectionHeight = sectionHeight + remaining end
                            else
                                local remaining = (bodyContentSize.y - self._tab_h) - totalSectionHeightL
                                if remaining > 0 then sectionHeight = sectionHeight + remaining end
                            end
                        end
                    ...
                    tabContent._max_height = math.max(totalSectionHeightL, totalSectionHeightR)
                    self._clip_box = nil
                else
